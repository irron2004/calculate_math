...
def _generate_arithmetic_problem_set(
    ...
    default_right_digits = config.right_digits or (
        digits[1] if len(digits) > 1 else default_left_digits
    )
    ...
    for index in range(count):
        ...
        if operator == "mul":
            ...
            dividend_digits = config.dividend_digits or default_left_digits or max(
                divisor_digits + 1, 2
            )
            ...
            if not config.allow_remainder:
                ...
                quotient_min, quotient_max = _resolve_digit_range(
                    quotient_digits, 2, 99
                )
                quotient = random.randint(quotient_min, max(quotient_min, quotient_max))
                dividend = divisor * quotient
                attempts = 0
                while attempts < max_attempts and not (
                    dividend_min <= dividend <= dividend_max
                ):
                    quotient = random.randint(
                        quotient_min, max(quotient_min, quotient_max)
                    )
                    dividend = divisor * quotient
                    attempts += 1
                if dividend_min > dividend_max:
                    dividend_min, dividend_max = _clamp_range(
            ...
            right_min, right_max = _resolve_digit_range(
                right_digits, default_min, default_max
            )
