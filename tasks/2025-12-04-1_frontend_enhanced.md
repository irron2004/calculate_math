...
### 1. Implement and demonstrate the responsive Skill Tree layout, StudentDashboard integration, tooltip/arrow fixes, and telemetry events on desktop/mobile with QA captures once backend is stable.
- **우선순위**: high
- **마감일**: TBD
- **출처**: qa_release

### 2. Add an npm `test` script or document the replacement command so dashboard/skill-tree unit tests can run in CI.
- **우선순위**: medium
- **마감일**: TBD
- **출처**: senior_engineer
