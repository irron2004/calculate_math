...
### 2. Replace SHA-256 password hashing with Argon2id or bcrypt (with pepper/strong cost) and migrate stored hashes accordingly.
- **우선순위**: high
- **마감일**: TBD
- **출처**: security
- **위험요인**: ['Password storage in app/routers/practice.py:90 still uses unsalted SHA-256, enabling offline brute force if the user table is leaked.', 'Session cookies are issued with SESSION_COOKIE_SECURE defaulting to false in app/config.py:155-158 and no SameSite enforcement, exposing tokens on insecure transports and to CSRF.', 'Login endpoint lacks rate limiting or tamper-proof audit logging, so credential stuffing can proceed undetected and without traceability.']
- **산출물**: [{'type': 'doc', 'uri': 'docs/THREAT_MODEL.md', 'note': 'Update with revised authentication and session management data flows once mitigations land.'}, {'type': 'doc', 'uri': 'docs/SECURITY_REPORT.md', 'note': 'Document bcrypt/Argon2 migration, cookie policy, and rate-limit validation results for release sign-off.'}]

### 3. Set SESSION_COOKIE_SECURE=True and SameSite=strict by default, and ensure issuance path enforces HTTPS plus short TTL.
- **우선순위**: high
- **마감일**: TBD
- **출처**: security
- **위험요인**: ['Password storage in app/routers/practice.py:90 still uses unsalted SHA-256, enabling offline brute force if the user table is leaked.', 'Session cookies are issued with SESSION_COOKIE_SECURE defaulting to false in app/config.py:155-158 and no SameSite enforcement, exposing tokens on insecure transports and to CSRF.', 'Login endpoint lacks rate limiting or tamper-proof audit logging, so credential stuffing can proceed undetected and without traceability.']
...
### 4. Restore the corrupted skill-tree/config modules with real code, add regression tests for missing-data fallbacks, and re-run make test/smoke.
- **우선순위**: high
- **마감일**: TBD
- **출처**: senior_engineer
